...
parser.add_argument("--input-weight", type=str, help="Weights file input")
parser.add_argument("--input-weight-scaled", type=str, help="Correctly scaled weights file input")
parser.add_argument("--output-norm", type=str, help="Const scale factor file")
...
parser.add_argument("--cat", type=str, help="Category", required=True)
...
reweight_hist = f.Get(args.cat + "/"+ args.var +"/histo_"+args.sample_to_reweight)
...
for ibin in range(1, nbins):
    bin_content = reweight_hist.GetBinContent(ibin)
    var_uncorrected[str(ibin)] = bin_content
    # print(ibin, xs[ibin-1], ys[ibin-1])
    reweight_hist.SetBinContent(ibin, bin_content * ys[ibin-1]) 
reweight_hist_integral_new = reweight_hist.Integral()
...
if args.input_weight_scaled:
    ...
    for ibin in range(1, nbins):
        ...
        bin_content = var_uncorrected[str(ibin)]
        # print(ibin, xs[ibin-1], ys[ibin-1])
        reweight_hist.SetBinContent(ibin, bin_content * ys_scaled[ibin-1]) 
    print(reweight_hist.Integral() / reweight_hist_integral)
